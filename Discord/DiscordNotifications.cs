...
namespace GTRC_Server_Basics.Discord
{
    public static class DiscordNotifications
    {
        ...
        static DiscordNotifications()
        {
            ...
            _ = DiscordBot.LoadEntryIdList(pathPendingAutoRNrs);
            _ = DiscordBot.LoadEntryIdList(pathPendingIsNotScorer);
        }
        ...
        public static async Task ShowNewRegistrations(Season season, List<Entry> newEntries, bool forceUpdateEntryConflicts = false)
        {
            ...
            if (respSerDis.Status == HttpStatusCode.OK)
            {
                ...
                List<Entry> entriesAutoGeneratedRaceNumber = (await DbApi.DynCon.Entry.UpdateRaceNumbers(season.Id)).List;
                _ = NotifyDriversAutoGeneratedRaceNumbers(entriesAutoGeneratedRaceNumber);
                if (entriesAutoGeneratedRaceNumber.Count > 0)
                {
                    message += "**Aufgrund der neuen Anmeldungen nachträglich angepasste Startnummern:**";
                    foreach (Entry entry in entriesAutoGeneratedRaceNumber) { message += "\n- " + await AddEntryToDiscordMessage(entry); }
                    message += "\n\n";
                }
