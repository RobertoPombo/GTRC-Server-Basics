...
namespace GTRC_Server_Basics.Discord
{
    public static class DiscordNotifications
    {
        ...
        public static async Task ShowSeasonSettingsViolations(Season season)
        {
            ...
            listEntries = (await DbApi.DynCon.Entry.GetAutoGeneratedRaceNumbers(season.Id)).List;
            if (listEntries.Count > 0)
            {
                message += "\n\n**Alle automatisch vergebenen Startnummern:**";
                foreach (Entry entry in listEntries) { message += "\n- " + await AddEntryToDiscordMessage(entry, true); }
            }
            listUsers = (await DbApi.DynCon.User.GetViolationsDiscordId(season.Id)).List;
            ...
            if (listEntries.Count > 0)
            {
                message += "\n\n**Entries mit mehr als " + season.MaxDriversPerEntryEvent.ToString() + " Fahrer";
            ...
            if (listEntries.Count > 0)
            {
                message += "\n\n**Entries mit wechselnden Fahrern:**";
                foreach (Entry entry in listEntries) { message += "\n- " + await AddEntryToDiscordMessage(entry); }
            }
            listEntries = (await DbApi.DynCon.Entry.GetViolationsDateStartRegistration(season.Id)).List;
            if (listEntries.Count > 0)
            {
                message += "\n\n**Anmeldungen vor dem " + bScripts.Date2String(season.DateStartRegistration.ToLocalTime(), "DD.MM.YY - hh:mm") + " Uhr:**";
            ...
            if (listEntries.Count > 0)
            {
                message += "\n\n**Anmeldungen nach dem " + bScripts.Date2String(season.DateEndRegistration.ToLocalTime(), "DD.MM.YY - hh:mm") + " Uhr:**";
                foreach (Entry entry in listEntries) { message += "\n- " + await AddEntryToDiscordMessage(entry, false, true); }
            }
            listTeams = (await DbApi.DynCon.Team.GetViolationsMinEntriesPerTeam(season.Id)).List;
            if (listTeams.Count > 0)
            {
                message += "\n\n**Teams mit weniger als " + season.MinEntriesPerTeam.ToString();
            ...
            if (listTeams.Count > 0)
            {
                message += "\n\n**Teams mit mehr als " + season.MaxEntriesPerTeam.ToString();
            ...
            else { message = "**" + messageHeader + "**" + message; }
